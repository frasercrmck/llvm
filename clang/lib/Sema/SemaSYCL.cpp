...
static bool CheckWorkGroupSize(SemaSYCL &S, const Expr *NSWIValue,
...
static bool InvalidWorkGroupSizeAttrs(SemaSYCL &S, const Expr *MGValue,
...
void SemaSYCL::CheckDeprecatedSYCLAttributeSpelling(const ParsedAttr &A,
  ...
  if (A.getKind() == ParsedAttr::AT_SYCLIntelInitiationInterval &&
    ...
    SemaRef.DiagnoseDeprecatedAttribute(A, "intel", "initiation_interval");
    return;
  }
  ...
  if (getLangOpts().getSYCLVersion() > LangOptions::SYCL_2017) {
    ...
    if (A.hasScope() && A.getScopeName()->isStr("cl")) {
      SemaRef.DiagnoseDeprecatedAttribute(A, "sycl", NewName);
      return;
    }
...
void SemaSYCL::handleIntelReqdSubGroupSizeAttr(Decl *D, const ParsedAttr &AL) {
  ...
  addIntelReqdSubGroupSize(D, AL, E);
}
...
SemaSYCL::mergeIntelNamedSubGroupSizeAttr(Decl *D,
  ...
  return IntelNamedSubGroupSizeAttr::Create(getASTContext(), A.getType(), A);
}
...
static bool checkWorkGroupSizeAttrExpr(SemaSYCL &S, Decl *D,
...
void SemaSYCL::handleSYCLDeviceAttr(Decl *D, const ParsedAttr &AL) {
  ...
  handleSimpleAttribute<SYCLDeviceAttr>(SemaRef, D, AL);
}
...
void SemaSYCL::handleSYCLDeviceIndirectlyCallableAttr(Decl *D,
  ...
  handleSimpleAttribute<SYCLDeviceIndirectlyCallableAttr>(SemaRef, D, AL);
}
...
void SemaSYCL::handleSYCLGlobalVarAttr(Decl *D, const ParsedAttr &AL) {
  ...
  handleSimpleAttribute<SYCLGlobalVarAttr>(SemaRef, D, AL);
}
...
void SemaSYCL::handleSYCLIntelBankBitsAttr(Decl *D, const ParsedAttr &A) {
  checkForDuplicateAttribute<SYCLIntelBankBitsAttr>(SemaRef, D, A);
...
void SemaSYCL::CheckSYCLAddIRAttributesFunctionAttrConflicts(Decl *D) {
  ...
  if (NumArgsWithoutFilter == 2) {
    auto NameValuePairs =
        AddIRFuncAttr->getAttributeNameValuePairs(getASTContext());
...
bool SemaSYCL::isAliasValid(unsigned BuiltinID,
                            [[maybe_unused]] StringRef AliasName) {
  constexpr llvm::StringLiteral Prefix = "__builtin_intel_sycl";
  return getASTContext().BuiltinInfo.getName(BuiltinID).starts_with(Prefix);
}
...
static bool isDeviceAspectType(const QualType Ty) {
...
void SemaSYCL::handleSYCLIntelMaxWorkGroupSizeAttr(Decl *D,
                                                   const ParsedAttr &AL) {
...
void SemaSYCL::handleSYCLIntelMinWorkGroupsPerComputeUnitAttr(
...
void SemaSYCL::handleSYCLIntelMaxWorkGroupsPerMultiprocessorAttr(
    Decl *D, const ParsedAttr &AL) {
