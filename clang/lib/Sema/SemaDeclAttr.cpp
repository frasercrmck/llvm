...
static void handleBuiltinAliasAttr(Sema &S, Decl *D,
  ...
  if ((IsAArch64 && !S.ARM().SveAliasValid(BuiltinID, AliasName)) ||
      ...
      (IsSYCL && !S.SYCL().isAliasValid(BuiltinID, AliasName)) ||
...
ProcessDeclAttribute(Sema &S, Scope *scope, Decl *D, const ParsedAttr &AL,
  ...
  case ParsedAttr::AT_SYCLIntelMaxWorkGroupSize:
    S.SYCL().handleSYCLIntelMaxWorkGroupSizeAttr(D, AL);
    break;
  case ParsedAttr::AT_SYCLIntelMinWorkGroupsPerComputeUnit:
    S.SYCL().handleSYCLIntelMinWorkGroupsPerComputeUnitAttr(D, AL);
    break;
  case ParsedAttr::AT_SYCLIntelMaxWorkGroupsPerMultiprocessor:
    S.SYCL().handleSYCLIntelMaxWorkGroupsPerMultiprocessorAttr(D, AL);
    break;
